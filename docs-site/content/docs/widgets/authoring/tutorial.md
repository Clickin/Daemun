...
description: Follow along with this guide to learn how to create a custom widget for Daemun. We'll cover the basic structure of a widget, how to use translations, and how to fetch data from an API.
---

In this guide, we'll walk through the process of creating a custom widget for Daemun. We'll cover the basic structure of a widget, how to use translations, and how to fetch data from an API. By the end of this guide, you'll have a solid understanding of how to build your own custom widget.

**Prerequisites:**

- Basic knowledge of React and JavaScript
- Familiarity with Daemun
...
Inside the `yourwidget` folder, create a new file named `widget.ts`. This file will contain the metadata for your widget.

Open the `widget.ts` file and add the following code:

```ts title="src/widgets/yourwidget/widget.ts"
...
Refer to the [translations guide](translations/) for more details. Daemun inherited a multilingual widget model from upstream, and we strongly encourage you to add translations for your widgets.

## Create the widget component

Create a new file for your widgets component, named `component.tsx`, in the `src/widgets/yourwidget` directory. We'll build the contents of the `component.tsx` file step by step.

First, we'll import the necessary dependencies:

```tsx title="src/widgets/yourwidget/component.tsx" linenums="1"
...
```tsx title="src/widgets/yourwidget/component.tsx" linenums="7"
...
```tsx title="src/widgets/yourwidget/component.tsx" linenums="8"
...
```tsx title="src/widgets/yourwidget/component.tsx" linenums="9"
...
```tsx title="src/widgets/yourwidget/component.tsx" linenums="10"
...
```tsx title="src/widgets/yourwidget/component.tsx" linenums="12"
if (error) {
...
```tsx title="src/widgets/yourwidget/component.tsx" linenums="16"
if (!data) {
  return (
...
```tsx title="src/widgets/yourwidget/component.tsx" linenums="26"
return (
  <Container service={service}>
...
Here's the complete `component.tsx` file:
...
## Add the widget to Daemun

To add your widget to Daemun, you need to register it in the `src/widgets/widgets.ts` file.

Open the `src/widgets/widgets.ts` file and import the `Component` from your widget's `component.tsx` file. Please keep the alphabetical order.
...
You also need to add the widget to the `components` object in the `src/widgets/components.ts` file.

Open the `src/widgets/components.ts` file and import the `Component` from your widget's `component.tsx` file.
...
You can now use your custom widget in Daemun. Open your `services.yaml` file and add a new service with the `yourwidget` widget.
...
That's it! You've successfully created a custom widget for Daemun.
