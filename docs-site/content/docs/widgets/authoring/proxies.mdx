---
title: Proxies Guide
description: Learn about proxy handlers in Daemun, and how to securely fetch data from an API.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Daemun includes a set of built-in proxy handlers that can be used to fetch data from an API. We will go over how to use these proxy handlers and briefly cover how to create your own.

## Available Proxy Handlers

Daemun comes with a few built-in proxy handlers that can be used to fetch data from an API. These handlers are located in the `utils/proxy/handlers` directory.
...
Proxy handlers are a complex topic and require a good understanding of TypeScript and the Daemun codebase. If you are new to Daemun, we recommend using the built-in proxy handlers.
