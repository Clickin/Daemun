...
description: Explore all the metadata properties that can be used to configure a widget in Daemun.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Here, we will go over how to create and configure Daemun widget metadata. Metadata is a TypeScript object that contains information about the widget, such as the API endpoint, proxy handler, and mappings. This metadata is used by Daemun to fetch data from the API and pass it to the widget.
...
The `proxyHandler` property is a function that will be used to make the API request. Daemun includes some built-in proxy handlers that can be used out of the box:

Here is an example of the generic proxy handler that makes unauthenticated requests to the specified API endpoint.

<Tabs>
  <TabItem label="widget.ts">
...
If you are looking to learn more about proxy handlers, please refer to the guide here: [Proxies Guide](proxies/).
...
The `mappings` or `allowedEndpoints` property is required for the widget to fetch data from more than a static URL. Daemun uses a whitelist approach to ensure that widgets only access allowed endpoints.
:::
