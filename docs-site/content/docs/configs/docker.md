...
Here is an example docker-compose file that will expose the docker socket, and then connect to it from the Daemun container:
...
In order to use the socket directly Daemun must be running as root
:::
...
Daemun features automatic service discovery for containers with the proper labels attached. For upstream compatibility, all configuration options can be applied using dot notation beginning with `homepage`.
...
When your Docker instance has been properly configured, this service will be automatically discovered and added to Daemun. **You do not need to specify the `server` or `container` values, as they will be automatically inferred.**
...
You can specify fields for e.g. the [CustomAPI](../widgets/services/customapi/) widget by using array-style dot notation:
...
For the automatic service discovery to discover all services it is important that Daemun should be deployed on a manager node. Set deploy requirements to the master node in your stack yaml config, e.g.
...
In order to detect every service within the Docker swarm it is necessary that service labels should be used and not container labels. Specify the `homepage.*` compatibility labels as:
