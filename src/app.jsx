...
function seedInitialQueryData(queryClient, initialQueryData) {
  if (!initialQueryData) return;

  Object.entries(initialQueryData).forEach(([url, data]) => {
    if (data !== undefined) {
      queryClient.setQueryData(["api", url], data);
    }
  });
}

function createQueryClient(initialQueryData) {
  ...
  seedInitialQueryData(queryClient, initialQueryData);
  return queryClient;
}

export function AppProviders({ children, initialQueryData, initialSettings }) {
  const [queryClient] = useState(() => createQueryClient(initialQueryData));
