...
// ponytail: upstream uses luxon; Daemun has no luxon dependency, dayjs is equivalent here
function buildSummary(backups, notifications, serverstate, progressstate) {
  ...
  backups.forEach((backup) => {
    ...
    const lastBackup = dayjs.utc(metadata.LastBackupFinished ?? "", "YYYYMMDD[T]HHmmss[Z]");
    const nextRun = dayjs(backup?.Schedule?.Time ?? "");
  ...
  return {
    ...summary,
    lastBackup: latestBackupTime?.utc().toISOString() ?? null,
    nextRun: nextRunTime?.utc().toISOString() ?? null,
  };
}
