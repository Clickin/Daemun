...
export type InertiaRootView = (page: InertiaPage, context: Context) => string | Promise<string>;
...
function defaultRootView(page: InertiaPage): string {
  return `<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
...
export function inertia(options: InertiaOptions = {}): MiddlewareHandler {
  ...
  return async function inertiaMiddleware(c, next) {
    ...
    c.setRenderer((component: string, props: UnknownRecord = {}) => {
