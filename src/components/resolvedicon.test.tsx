...
vi.mock("utils/image", () => ({
  default: ({ src, alt }) => <div data-testid="resolved-image" data-src={src} data-alt={alt} />,
}));
...
describe("components/resolvedicon", () => {
  it("renders direct URL icons via utils/image", () => {
    ...
    expect(screen.getByTestId("resolved-image").getAttribute("data-src")).toBe("http://example.com/x.png");
  });

  it("renders relative URL icons via utils/image", () => {
    renderWithContexts(<ResolvedIcon icon="/icons/x.png" alt="x" />);
    expect(screen.getByTestId("resolved-image").getAttribute("data-src")).toBe("/icons/x.png");
  });

  it("renders selfh.st icons for sh- prefix with extension", () => {
    renderWithContexts(<ResolvedIcon icon="sh-test.webp" alt="x" />);
    expect(screen.getByTestId("resolved-image").getAttribute("data-src")).toContain("/webp/test.webp");
  });

  it("renders selfh.st icons as svg or png based on file extension", () => {
    renderWithContexts(<ResolvedIcon icon="sh-test.svg" alt="x" />);
    expect(screen.getByTestId("resolved-image").getAttribute("data-src")).toContain("/svg/test.svg");

    renderWithContexts(<ResolvedIcon icon="sh-test.png" alt="x" />);
    expect(screen.getAllByTestId("resolved-image")[1].getAttribute("data-src")).toContain("/png/test.png");
  });
  ...
  it("falls back to dashboard-icons for .svg", () => {
    ...
    expect(screen.getByTestId("resolved-image").getAttribute("data-src")).toContain("/dashboard-icons/svg/foo.svg");
  });

  it("falls back to dashboard-icons for .webp and .png", () => {
    renderWithContexts(<ResolvedIcon icon="foo.webp" />);
    expect(screen.getAllByTestId("resolved-image")[0].getAttribute("data-src")).toContain(
      "/dashboard-icons/webp/foo.webp",
    );

    renderWithContexts(<ResolvedIcon icon="foo.png" />);
    expect(screen.getAllByTestId("resolved-image")[1].getAttribute("data-src")).toContain(
